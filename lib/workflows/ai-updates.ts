import type { WorkflowSpec, WorkflowSectionKey, WorkflowStep, WorkflowStepType } from "./types";

export type WorkflowUpdates = {
  summary?: string | null;
  steps?: Array<{
    id: string;
    title?: string;
...
export function applyWorkflowUpdates(workflow: WorkflowSpec, updates: WorkflowUpdates): WorkflowSpec {
  ...
  if (updates.steps && updates.steps.length > 0) {
    const normalizedSteps = updates.steps.map((incoming, index) =>
      normalizeIncomingStep(incoming, workflow.steps[index])
    );
  ...
  if (updates.sections) {
    const sectionMap = new Map<WorkflowSectionKey, number>();
    sections.forEach((section, index) => {
      sectionMap.set(section.key, index);
    });

    for (const key of Object.keys(updates.sections) as WorkflowSectionKey[]) {
  ...
  return {
    ...workflow,
    summary: summaryChanged ? summary : workflow.summary,
    steps: stepsChanged ? steps : workflow.steps,
    sections: sectionsChanged ? sections : workflow.sections,
  };
}

function normalizeIncomingStep(
  incoming: NonNullable<WorkflowUpdates["steps"]>[number],
...
function normalizeStepType(type?: string | null, fallback?: WorkflowStepType): WorkflowStepType {
  if (!type) {
  ...
  if ((["Trigger", "Action", "Decision", "Exception", "Human"] as WorkflowStepType[]).includes(type as WorkflowStepType)) {
...
// Legacy aliases
export type BlueprintUpdates = WorkflowUpdates;
export const applyBlueprintUpdates = applyWorkflowUpdates;
