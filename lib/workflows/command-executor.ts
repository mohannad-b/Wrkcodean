...
export function executeCommand(blueprint: Blueprint, command: ParsedCommand): CommandResult {
  try {
    if (command.type === "unknown") {
      return { success: false, blueprint, workflow: blueprint, message: "", error: "Unknown command", auditEvents: [] };
    }
    ...
    switch (command.type) {
      ...
      default:
        return {
          ...
          workflow: blueprint,
          message: "",
          error: `Command ${command.type} not supported`,
          auditEvents: [],
        };
    }
    ...
    return {
      ...
      workflow: { ...numbered, updatedAt: new Date().toISOString() },
      message,
      auditEvents,
    };
  } catch (error) {
