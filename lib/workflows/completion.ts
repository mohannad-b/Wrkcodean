import { WORKFLOW_SECTION_KEYS, type WorkflowSectionKey, type WorkflowSpec } from "./types";

export interface WorkflowSectionCompletion {
...
export interface WorkflowCompletionState {
  ...
  sections: WorkflowSectionCompletion[];
...
const SECTION_COMPLETION_TARGETS: Partial<Record<WorkflowSectionKey, number>> = {
...
const createEmptySectionProgress = (): WorkflowSectionCompletion[] =>
...
export function getWorkflowCompletionState(workflow: WorkflowSpec | null | undefined): WorkflowCompletionState {
  if (!workflow) {
    return {
      score: 0,
  ...
  const summaryLength = hasMeaningfulContent(workflow.summary, 12) ? workflow.summary.trim().length : 0;
  const summaryProgress = clamp(summaryLength / MIN_SUMMARY_LENGTH);
  const hasTrigger = workflow.steps.some((step) => step.type === "Trigger");
  const hasAction = workflow.steps.some((step) => step.type === "Action");
  ...
  const sectionContentMap = new Map<WorkflowSectionKey, string>();
  workflow.sections.forEach((section) => {
    sectionContentMap.set(section.key, section.content?.trim() ?? "");
  });

  const contentProgressLookup = new Map<WorkflowSectionKey, number>();
  WORKFLOW_SECTION_KEYS.forEach((key) => {
  ...
  const derivedSectionSignals: Record<WorkflowSectionKey, number> = {
  ...
  const sectionProgress = WORKFLOW_SECTION_KEYS.map<WorkflowSectionCompletion>((key) => {
    if (key === "flow_complete") {
...
export type BlueprintSectionCompletion = WorkflowSectionCompletion;
export type BlueprintCompletionState = WorkflowCompletionState;
export const getBlueprintCompletionState = getWorkflowCompletionState;
