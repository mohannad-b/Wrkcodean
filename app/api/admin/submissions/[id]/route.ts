...
import { getSubmissionDetail } from "@/lib/services/submissions";
...
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    ...
    const detail = await getSubmissionDetail(session.tenantId, params.id);
    ...
    if (detail) {
      return NextResponse.json({
        submission: {
          ...
          status: fromDbAutomationStatus(detail.submission.status),
    ...
    // Fallback: treat ID as an automation version ID (for automation requests without a submission row).
