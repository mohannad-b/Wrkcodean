...
async function parsePayload(request: Request): Promise<CreateSubmissionPayload> {
  try {
    return (await request.json()) as CreateSubmissionPayload;
  } catch {
    return {};
  }
}
...
export async function GET() {
  try {
    ...
    const isAdmin = can(session, "admin:submission:read", { type: "submission", tenantId: session.tenantId });
    const canViewTenant = can(session, "automation:read", { type: "automation", tenantId: session.tenantId });
    if (!isAdmin && !canViewTenant) {
      throw new ApiError(403, "Forbidden");
    }

    const items = await listSubmissionsForTenant(session.tenantId);
    const seenVersionIds = new Set(items.map((item) => item.version?.id).filter(Boolean) as string[]);
    const requests = await listSubmissionRequestsForTenant(session.tenantId, seenVersionIds);
    ...
    return NextResponse.json({
      submissions: combined.map((item) => ({
        id: item.submission.id,
        name: item.submission.name,
        status: fromDbAutomationStatus(item.submission.status),
...
export async function POST(request: Request) {
  try {
    ...
    if (!can(session, "admin:submission:write", { type: "submission", tenantId: session.tenantId })) {
      throw new ApiError(403, "Forbidden");
    }
    ...
    return NextResponse.json(
      {
        submission: {
          id: submission.id,
          status: fromDbAutomationStatus(submission.status),
          automationId: submission.automationId,
          automationVersionId: submission.automationVersionId,
        },
      },
      { status: 201 }
    );
