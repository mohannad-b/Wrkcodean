...
const USERS = [
  { id: 1, name: "Sarah Chen", email: "sarah@company.com", role: "Owner", avatar: "https://github.com/shadcn.png" },
...
const VERSIONS = [
  { id: "v1.1", status: "building", date: "Nov 12, 2023", summary: "Delta: +3 steps, +1 integration", author: "Sarah Chen" },
  { id: "v1.0", status: "active", date: "Oct 24, 2023", summary: "Initial Release", author: "Sarah Chen" },
  { id: "v0.9", status: "superseded", date: "Oct 10, 2023", summary: "Beta Release", author: "Mike Ross" },
];

const SYSTEMS = [
  { id: 1, name: "Salesforce", icon: "cloud", status: "connected", usage: "Create Opp, Update Contact" },
  { id: 2, name: "Xero", icon: "file-text", status: "attention", usage: "Create Draft Bill" },
  { id: 3, name: "Google Drive", icon: "hard-drive", status: "connected", usage: "Store PDF Reports" },
];

type SettingsTab = 'general' | 'permissions' | 'systems' | 'billing' | 'versions' | 'notifications' | 'data';
...
export const AutomationSettings: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'systems': return <SystemsSettings />;
      case 'billing': return <BillingSettings />;
      case 'versions': return <VersionsSettings />;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
               ...
               <div className="flex flex-wrap gap-2 p-2 border border-gray-200 rounded-md min-h-[42px]">
                  <Badge variant="secondary" className="bg-gray-100 hover:bg-gray-200 text-gray-700">Finance</Badge>
                  <Badge variant="secondary" className="bg-gray-100 hover:bg-gray-200 text-gray-700">Accounts Payable</Badge>
                  <button className="text-xs text-gray-400 hover:text-[#E43632] px-2 font-medium">+ Add</button>
               </div>
            </div>
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-100 text-xs text-gray-500 flex items-start gap-2">
          ...
          <p>Note: Only <strong>Owners</strong> can delete this automation. <strong>Editors</strong> can deploy new versions.</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-lg p-4 flex items-start gap-3 text-amber-800">
         ...
         <div className="text-sm">
           <span className="font-bold">Action Required:</span> One or more systems need reconnection. Please update credentials to ensure automation continuity.
         </div>
      </div>

      <div className="grid gap-4">
        {SYSTEMS.map((system) => (
          <div key={system.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center justify-between">
             <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-gray-50 rounded-lg flex items-center justify-center text-gray-500 border border-gray-100">
                   {/* Placeholder icons based on mock data strings */}
                   {system.name === 'Salesforce' && <span className="font-bold text-blue-500">SF</span>}
                ...
                <div>
                   <div className="flex items-center gap-2">
                     ...
                     {system.status === 'connected' ? (
                       <Badge variant="outline" className="text-emerald-600 bg-emerald-50 border-emerald-200 text-[10px] gap-1 px-1.5 py-0 h-5">
                         <CheckCircle2 size={10} /> Connected
                       </Badge>
                     ) : (
                       <Badge variant="outline" className="text-amber-600 bg-amber-50 border-amber-200 text-[10px] gap-1 px-1.5 py-0 h-5">
             ...
             <Button 
               ...
               className={system.status === 'connected' 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
         ...
         <div className="p-6 border-b border-gray-100 bg-gray-50/50">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
               <div>
                  <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-1">Active Plan (v1.0)</h4>
               ...
               <div className="flex items-center gap-8 text-sm">
                  ...
                  <Button variant="link" className="text-[#E43632] h-auto p-0">View Signed Quote</Button>
               </div>
            </div>
         </div>
         ...
         <div className="p-6 border-t border-gray-100 flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" className="text-gray-600">Update Payment Method</Button>
         </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-lg p-4 flex items-start gap-3 text-amber-800">
         <AlertTriangle size={18} className="mt-0.5 shrink-0" />
         <div className="text-sm">
           <span className="font-bold">Note:</span> You have a draft version (v1.1) in progress. Changes may require re-approval and updated pricing before going live.
         </div>
      </div>
    </div>
  );
...
// --- 5. Versions ---
const VersionsSettings = () => {
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active': return <Badge className="bg-emerald-500 hover:bg-emerald-600 text-white border-none">Active Version</Badge>;
      case 'building': return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Build in Progress</Badge>;
      case 'superseded': return <Badge variant="outline" className="bg-gray-100 text-gray-400 border-gray-200">Superseded</Badge>;
      default: return null;
    }
  };
      ...
      <div className="space-y-4">
         {VERSIONS.map((version) => (
            <div key={version.id} className={cn(
              ...
              version.status === 'active' ? "border-emerald-200 shadow-emerald-500/5 shadow-sm ring-1 ring-emerald-100" : "border-gray-200 shadow-sm opacity-90 hover:opacity-100"
            )}>
               ...
               <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" className={cn(
                    version.status === 'active' ? "bg-emerald-600 hover:bg-emerald-700" : "bg-[#0A0A0A] hover:bg-gray-800"
                  )}>
                    {version.status === 'building' ? 'Open Build Status' : 'View Configuration'} <ChevronRight size={14} className="ml-1" />
                  </Button>
               </div>
            </div>
         ))}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
         <div className="grid grid-cols-1 divide-y divide-gray-100">
            {[
              { label: 'Exceptions & Errors', desc: 'When automation fails or needs human review', critical: true },
               ...
               <div key={idx} className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                     <h4 className="font-bold text-[#0A0A0A] flex items-center gap-2">
                       {item.label}
                       {item.critical && <Badge variant="outline" className="text-[10px] text-red-600 border-red-100 bg-red-50 px-1 py-0">Critical</Badge>}
                     </h4>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-2">
         <div>
            ...
            <p className="text-sm text-gray-500">Manage data retention, privacy, and compliance settings.</p>
         </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-8">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
               <h4 className="text-sm font-bold text-[#0A0A0A] border-b border-gray-100 pb-2">Data Sensitivity</h4>
            ...
            <div className="space-y-4">
               <h4 className="text-sm font-bold text-[#0A0A0A] border-b border-gray-100 pb-2">Access Controls</h4>
         ...
         <div className="bg-gray-50 rounded-lg p-4 border border-gray-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
               ...
               <div>
                  ...
                  <p className="text-xs text-gray-500">View all administrative actions and access events.</p>
               </div>
            </div>
