...
const TASKS: Task[] = [
  {
    ...
    priority: 'blocking',
    date: '2 hours ago',
    description: 'Version 1.1 adds 3 new steps and increases unit cost. Signature required to proceed with build.',
    origin: 'AI System',
  ...
  },
  {
    ...
    priority: 'high',
    date: 'Today, 9:41 AM',
    description: 'The AI confidence score for "Vendor Name" was 42% (Threshold: 80%). Please manually verify.',
  ...
  },
  {
    ...
    priority: 'blocking',
    date: 'Yesterday',
    description: 'Authentication token for Xero has expired. Automations using this connection are paused.',
    origin: 'System Monitor',
  },
  {
    id: 't4',
    ...
    priority: 'normal',
    date: 'Oct 26',
    description: 'You specified "Manager Approval" but did not define the threshold amount. Please update configuration.',
    origin: 'AI System',
...
const TaskIcon = ({ type }: { type: TaskType }) => {
  switch (type) {
    case 'approval': return <FileSignature className="text-[#E43632]" />;
    case 'version': return <FileSignature className="text-[#E43632]" />;
    case 'exception': return <AlertTriangle className="text-amber-500" />;
    case 'access': return <Key className="text-red-500" />;
    case 'info': return <HelpCircle className="text-blue-500" />;
    case 'review': return <CheckCircle2 className="text-emerald-500" />;
    default: return <Zap />;
  }
};
...
const PriorityBadge = ({ priority }: { priority: Priority }) => {
  switch (priority) {
    case 'blocking': 
      return <Badge className="bg-red-100 text-red-700 border-red-200 hover:bg-red-100">Blocking</Badge>;
    case 'high': 
      return <Badge className="bg-amber-100 text-amber-800 border-amber-200 hover:bg-amber-100">High Priority</Badge>;
    default: 
      return <Badge variant="outline" className="text-gray-500 border-gray-200">Normal</Badge>;
  }
};
              ...
              {filteredTasks.map(task => (
                <div 
                  ...
                  className={cn(
                    ...
                    selectedTaskId === task.id ? "border-[#E43632] ring-1 ring-[#E43632]" : "border-gray-200"
                  )}
                >
                   ...
                   <div className="absolute right-5 bottom-5 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button size="sm" variant="outline" className="text-xs h-8">Open</Button>
                   </div>
                </div>
              ))}
             ...
             <ScrollArea className="flex-1">
                <div className="p-8 space-y-8">
                   ...
                   <div className="space-y-4">
                      ...
                      <h2 className="text-2xl font-bold text-[#0A0A0A] leading-tight">{selectedTask.title}</h2>
                   </div>

                   {/* Metadata Grid */}
                   <div className="grid grid-cols-2 gap-6 bg-gray-50 p-4 rounded-xl border border-gray-100">
                      <div>
                         <label className="text-xs font-bold text-gray-400 uppercase mb-1 block">Automation</label>
                         <p className="text-sm font-bold text-[#0A0A0A]">{selectedTask.automationName}</p>
                      </div>
                      <div>
                         <label className="text-xs font-bold text-gray-400 uppercase mb-1 block">Version</label>
                         <p className="text-sm font-bold text-[#0A0A0A]">{selectedTask.version}</p>
                      </div>
                      <div>
                         <label className="text-xs font-bold text-gray-400 uppercase mb-1 block">Step</label>
                         <p className="text-sm font-medium text-gray-600">{selectedTask.stepName || 'N/A'}</p>
                      </div>
                      <div>
                         <label className="text-xs font-bold text-gray-400 uppercase mb-1 block">Requested By</label>
                   ...
                   {/* Blueprint Link */}
                   <Button variant="outline" className="w-full justify-between border-gray-200 text-gray-700 h-12">
                      <span className="flex items-center gap-2"><Zap size={16} /> View in Blueprint</span>
             ...
             <div className="p-6 border-t border-gray-200 bg-gray-50 shrink-0 space-y-3">
                ...
                {selectedTask.type === 'approval' || selectedTask.type === 'version' ? (
                   <div className="grid grid-cols-2 gap-3">
                      <Button variant="outline" className="border-gray-200 text-gray-700">Reject</Button>
                   ...
                   <div className="grid grid-cols-2 gap-3">
                      <Button variant="outline" className="border-gray-200 text-gray-700">Ignore</Button>
                ...
                <div className="text-center">
                   <button className="text-xs text-gray-400 hover:text-gray-600 font-medium">Snooze for 1 hour</button>
                </div>
             </div>
          </motion.div>
        )}
