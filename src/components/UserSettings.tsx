...
const CONNECTED_ACCOUNTS = [
  { id: 1, name: "Google Drive", status: "connected", icon: LayoutTemplate, lastUsed: "2 hours ago" },
...
const SESSIONS = [
  { id: 1, device: "MacBook Pro 16\"", location: "San Francisco, US", ip: "192.168.1.1", active: true, time: "Current Session" },
  { id: 2, device: "iPhone 14 Pro", location: "San Francisco, US", ip: "192.168.1.45", active: false, time: "Active 2h ago" },
  { id: 3, device: "Chrome / Windows", location: "Austin, US", ip: "10.0.0.4", active: false, time: "Active 1d ago" },
];
    ...
    <div className="bg-gradient-to-br from-[#0A0A0A] to-[#222] rounded-xl p-6 text-white shadow-lg flex items-center justify-between">
       <div>
          <div className="flex items-center gap-2 mb-2">
             ...
             <span className="text-sm font-bold uppercase tracking-wider text-gray-400">Pending Tasks</span>
          </div>
          ...
          <div className="mt-2 flex gap-2">
             <Badge variant="outline" className="bg-white/10 border-white/10 text-white hover:bg-white/20">2 Approval Requests</Badge>
             <Badge variant="outline" className="bg-white/10 border-white/10 text-white hover:bg-white/20">2 Reviews</Badge>
          </div>
       </div>
       <Button variant="outline" className="border-white/20 text-black hover:bg-white hover:text-black">
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8 space-y-8">
       ...
       <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
             ...
             <Button size="sm" variant="outline" className="mt-2 text-xs">Upload New</Button>
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
             ...
             <div className="relative">
                <Input defaultValue="adrian@wrk.com" readOnly className="bg-gray-50 pr-12 text-gray-500" />
                <div className="absolute right-3 top-1/2 -translate-y-1/2">
                   <Badge variant="secondary" className="text-[10px] h-5 bg-gray-200 text-gray-600">SSO</Badge>
                </div>
             </div>
          </div>
       ...
       <div className="flex justify-end pt-4">
          <Button className="bg-[#E43632] hover:bg-[#C12E2A] text-white">Save Changes</Button>
       </div>
    </div>
  </div>
);
  ...
  <div className="space-y-6">
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-8">
       ...
       <div className="grid grid-cols-3 gap-4 bg-gray-50 p-4 rounded-lg">
          ...
          <div className="col-span-3 md:col-span-2 flex flex-wrap gap-6">
             <div className="flex items-center gap-2">
                ...
                <label htmlFor="email-notif" className="text-sm font-medium flex items-center gap-2"><Mail size={14} /> Email</label>
             </div>
             <div className="flex items-center gap-2">
                <Switch defaultChecked id="slack-notif" />
                <label htmlFor="slack-notif" className="text-sm font-medium flex items-center gap-2"><Slack size={14} /> Slack</label>
             </div>
             <div className="flex items-center gap-2">
                <Switch defaultChecked id="inapp-notif" />
                <label htmlFor="inapp-notif" className="text-sm font-medium flex items-center gap-2"><Bell size={14} /> In-App</label>
             </div>
          </div>
       </div>
       ...
       <div className="space-y-4">
          ...
          {[
             { label: "Approval Requests", desc: "When a quote or automation needs your approval", default: true },
             { label: "Review Requests", desc: "When you are tagged for a peer review", default: true },
             { label: "Missing Information", desc: "When a workflow is blocked by missing data", default: true },
             { label: "Exceptions & Errors", desc: "Critical failures in your owned automations", default: true },
             { label: "Version Approved", desc: "When a version you created is approved", default: false },
             { label: "Build Completed", desc: "When an automation build finishes", default: true },
             { label: "Access Issues", desc: "When a user requests access to your resources", default: false },
          ].map((item, i) => (
       ...
       <div className="grid grid-cols-1 divide-y divide-gray-100">
          {CONNECTED_ACCOUNTS.map((acc) => (
             <div key={acc.id} className="p-6 flex items-center justify-between hover:bg-gray-50 transition-colors">
                   ...
                   <div>
                      <div className="flex items-center gap-3">
                         ...
                         {acc.status === 'connected' ? (
                            <Badge variant="outline" className="text-emerald-600 bg-emerald-50 border-emerald-200 text-[10px]">Connected</Badge>
                         ) : (
                            <Badge variant="outline" className="text-red-600 bg-red-50 border-red-200 text-[10px]">Needs Reauth</Badge>
                         )}
                      </div>
                ...
                <div className="flex items-center gap-3">
                   {acc.status === 'expired' && (
                      <Button size="sm" className="bg-[#E43632] hover:bg-[#C12E2A] text-white">Reauthorize</Button>
                   )}
                   <Button variant="outline" size="sm" className="text-gray-600 border-gray-200 hover:text-red-600 hover:bg-red-50 hover:border-red-200">
                      Disconnect
                   </Button>
                </div>
             </div>
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-8">
       ...
       <div className="flex items-start gap-4 p-4 bg-blue-50 border border-blue-100 rounded-lg">
          ...
          <div>
             ...
             <p className="text-xs text-blue-700 mt-1">
                Your account is managed via your organization's Identity Provider (Okta). Password changes must be handled there.
             </p>
          </div>
       </div>
       ...
       <div className="space-y-4">
          ...
          <div className="flex items-center justify-between py-2">
             ...
             <Button variant="outline" className="text-emerald-600 border-emerald-200 bg-emerald-50">Enabled</Button>
          </div>
       </div>
       ...
       <div className="space-y-4 pt-2">
          <div className="flex items-center justify-between border-b border-gray-100 pb-2">
             ...
             <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50 h-8 text-xs">
       ...
       <div className="space-y-6">
          <div>
             ...
             <p className="text-sm text-gray-500 mt-1 max-w-xl">
                Permanently remove your account and all of its content from the WRK platform. This action is not reversible, so please continue with caution.
             </p>
          </div>
          ...
          <div className="p-4 bg-red-50 border border-red-100 rounded-lg text-sm text-red-800 flex items-start gap-3">
             ...
             <div>
                ...
                <p className="text-xs mt-1 text-red-700">
                   Since you are a Workspace Admin, you must transfer ownership of <strong>Acme Corp</strong> before deleting your account.
                </p>
             </div>
          </div>
