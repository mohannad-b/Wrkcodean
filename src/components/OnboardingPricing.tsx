...
export const OnboardingPricing: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  const getUnitPrice = (v: number) => {
    if (v >= 10000) return 0.10;
    if (v >= 5000) return 0.15;
    if (v >= 2500) return 0.20;
    return 0.25;
  };
            ...
            <div className="lg:col-span-2 space-y-6">
                  ...
                  <div className="flex justify-between items-start">
                     <div>
                        <h3 className="font-bold text-[#0A0A0A] text-lg flex items-center gap-2">
                           One-Time Build Fee
                           <Badge variant="secondary" className="bg-gray-100 text-gray-600 font-normal text-[10px]">Refundable</Badge>
                        </h3>
                     ...
                     <div className="text-right">
                        <span className="block text-2xl font-bold text-[#0A0A0A]">${buildFee.toLocaleString()}</span>
                     </div>
                  </div>
               </Card>
               ...
               <Card className="p-6 border-gray-200 shadow-sm space-y-6 bg-white relative overflow-hidden">
                  ...
                  <div>
                     <h3 className="font-bold text-[#0A0A0A] text-lg mb-1 flex items-center gap-2">
                        Recurring Usage
                        <span className="text-xs font-normal text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Post-Launch</span>
                     </h3>
                  ...
                  <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                     <div className="flex justify-between items-end mb-6">
                        <div>
                           ...
                           <div className="flex items-baseline gap-2">
                              <span className="text-3xl font-bold text-[#0A0A0A]">{currentVol.toLocaleString()}</span>
                              <span className="text-sm text-gray-500 font-medium">results / mo</span>
                           </div>
                        </div>
                        <div className="text-right">
                           <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">Unit Price</span>
                           <div className="flex items-baseline gap-1 justify-end">
                              <span className="text-3xl font-bold text-[#E43632]">${unitPrice.toFixed(2)}</span>
                     ...
                     <div className="grid grid-cols-4 text-center gap-2 text-[10px] text-gray-400 mb-2">
                        <div className={currentVol < 2500 ? "text-[#E43632] font-bold" : ""}>{'< 2.5k'}</div>
                        <div className={currentVol >= 2500 && currentVol < 5000 ? "text-[#E43632] font-bold" : ""}>{'2.5k+'}</div>
                        <div className={currentVol >= 5000 && currentVol < 10000 ? "text-[#E43632] font-bold" : ""}>{'5k+'}</div>
                        <div className={currentVol >= 10000 ? "text-[#E43632] font-bold" : ""}>{'10k+'}</div>
                     </div>
                     <div className="flex w-full h-2 rounded-full overflow-hidden bg-gray-200 mb-6">
                        <div className={`flex-1 transition-colors ${currentVol < 2500 ? "bg-[#E43632]" : "bg-gray-300"}`} />
                        <div className={`flex-1 transition-colors ${currentVol >= 2500 && currentVol < 5000 ? "bg-[#E43632]" : "bg-gray-300"}`} />
                        <div className={`flex-1 transition-colors ${currentVol >= 5000 && currentVol < 10000 ? "bg-[#E43632]" : "bg-gray-300"}`} />
                        <div className={`flex-1 transition-colors ${currentVol >= 10000 ? "bg-[#E43632]" : "bg-gray-300"}`} />
                     </div>

                     <div className="flex items-start gap-3 pt-4 border-t border-gray-200">
                        <Info size={16} className="text-blue-500 mt-0.5 shrink-0" />
                        <p className="text-xs text-gray-500 leading-relaxed">
                           <strong>How billing works:</strong> We bill your estimated spend (${monthlyEst}) in advance each month. Actual usage is deducted from this balance. Any unused credits roll over to the next month automatically.
                        </p>
                     </div>
                  </div>
               </Card>
               ...
               <Card className="p-6 border-gray-200 shadow-sm bg-white">
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-4">
                     <div>
                        <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Cardholder Name</label>
                        <Input 
                     ...
                     <div>
                        <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Card Number</label>
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                           <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Expiry</label>
                           <Input 
                              placeholder="MM/YY" 
                              className="bg-white"
                              value={expiry}
                  ...
                  <div className="p-6 space-y-6">
                     <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                           ...
                           <span className="font-bold text-[#0A0A0A]">{new Date().toLocaleDateString()}</span>
                        </div>
                     </div>
                     ...
                     <div className="space-y-3">
                        ...
                        <div className="flex justify-between text-sm text-green-600">
                           <span className="flex items-center gap-1"><Zap size={12}/> Initial Credits</span>
                     ...
                     <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 text-xs text-gray-500 leading-relaxed">
                        <strong>Refund Policy:</strong> The build fee is fully refundable if we cannot successfully build your requested automation.
                     </div>

                     {/* Signature Field */}
                     <div className="mt-6">
                        <label className="text-xs font-bold text-gray-400 uppercase mb-2 block">Sign to Confirm</label>
                        <div className="relative">
                           <Input 
