...
export const StudioChat: React.FC<StudioChatProps> = ({ isContributorMode = false, onAiCommand }) => {
    ...
    isContributorMode 
      ? [
          ...
          },
          {
            id: '2',
            role: 'ai',
            content: "Could you please upload a screenshot of the invoice approval screen in NetSuite? This will help us understand the UI structure.",
            timestamp: '10:00 AM'
          }
        ]
      : [
          ...
          },
          {
            id: '3',
            role: 'ai',
            content: "I've drafted a flow: Email Trigger -> PDF Extraction -> Xero Draft. I inferred the email subject format as 'Invoice #[Number]'. Is that correct?",
            timestamp: '10:01 AM',
    ...
    <div className="flex flex-col h-full bg-[#F9FAFB] border-r border-gray-200 overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-200 bg-white flex flex-col gap-3 shadow-sm z-10">
        <div className="flex items-center gap-2">
           <div className="bg-gradient-to-br from-[#E43632] to-[#FF5F5F] text-white p-1.5 rounded-lg shadow-sm animate-pulse-subtle">
           ...
           {isContributorMode && (
             <Badge variant="secondary" className="ml-auto text-[10px] bg-blue-50 text-blue-600 border-blue-100 px-2 py-0.5">Contributor Mode</Badge>
           )}
        </div>
        ...
        {isContributorMode && (
          <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-3 space-y-3 mt-1">
             <div className="flex items-center justify-between">
               <p className="text-[10px] font-bold text-blue-800 uppercase tracking-wider">Your Tasks (2)</p>
      ...
      <div className="p-4 bg-white border-t border-gray-200">
        ...
        {!isContributorMode && messages.length < 3 && (
           <div className="mb-3 px-3 py-2 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-between group cursor-pointer hover:bg-red-50 hover:border-red-100 transition-colors" onClick={() => setInput("Our sales process starts when a lead comes from Facebook ads...")}>
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            placeholder={isContributorMode ? "Reply to task..." : "Describe your workflow change..."}
...
const QuickAction = ({ icon: Icon, label }: { icon: any, label: string }) => (
  <button className="flex flex-col items-center justify-center gap-1.5 py-2.5 bg-white border border-gray-200 rounded-xl hover:border-[#E43632] hover:text-[#E43632] hover:bg-[#E43632]/5 transition-all group shadow-sm">
    ...
    <span className="text-[10px] font-bold text-gray-600 group-hover:text-[#E43632] transition-colors">{label}</span>
  </button>
);

const TaskItem = ({ label, type }: { label: string, type: 'upload' | 'connect' }) => (
  <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-blue-100 shadow-sm group hover:border-blue-300 transition-colors cursor-pointer">
    <div className="flex items-center gap-2">
      <div className={`p-1.5 rounded-md ${type === 'upload' ? 'bg-purple-50 text-purple-600' : 'bg-amber-50 text-amber-600'}`}>
