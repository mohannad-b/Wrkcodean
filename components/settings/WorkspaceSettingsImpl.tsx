...
const AUDIT_LOGS = [
  ...
  {
    id: 2,
    action: 'Invited "Mike Ross" (Editor)',
    user: "Sarah Chen",
    time: "Yesterday",
    ip: "192.168.1.1",
  },
      ...
      <div className="flex-1 overflow-y-auto p-8 md:p-12">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            {activeTab === "members" && <MembersSettings onGoToTeams={handleGoToTeams} />}
            {activeTab === "systems" && (
              <SystemsSettings
            ...
            {activeTab === "security" && <SecuritySettings onExportCSV={handleExportCSV} />}
            {activeTab === "branding" && (
              <BrandingSettings onPublish={handleSave} isSaving={isSaving} />
            )}
          </motion.div>
        </div>
      </div>
...
const MembersSettings = ({ onGoToTeams }: { onGoToTeams?: () => void } = {}) => (
  <div className="space-y-6">
    <div className="bg-gradient-to-r from-[#0A0A0A] to-[#333] rounded-xl p-8 text-white shadow-lg relative overflow-hidden">
      <div className="relative z-10">
        ...
        <p className="text-gray-300 mb-6 max-w-lg">
          Control access, assign roles, and manage workspace permissions for all your team members
          in the dedicated Teams view.
        </p>
        <div className="flex items-center gap-4">
          <Button
          ...
          <div className="flex -space-x-2">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="w-8 h-8 rounded-full bg-gray-600 border-2 border-[#0A0A0A] flex items-center justify-center text-xs font-bold"
              >
                {String.fromCharCode(64 + i)}
              </div>
            ))}
            <div className="w-8 h-8 rounded-full bg-gray-700 border-2 border-[#0A0A0A] flex items-center justify-center text-xs text-gray-300">
              +5
            </div>
          </div>
        </div>
...
const SystemsSettings = ({
  onTestConnection,
  onReconnect,
}: { onTestConnection?: (name: string) => void; onReconnect?: (name: string) => void } = {}) => {
  const handleAddSystem = () => {
    // TODO: Open system picker modal
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-2">
        ...
        <Button className="bg-[#0A0A0A] hover:bg-gray-800 text-white" onClick={handleAddSystem}>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex gap-3 text-blue-800">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700">
            Connections authenticated here are available to all workspace Editors. Use specific
            automation-level overrides for restricted accounts.
          </p>
        </div>
      </div>
            ...
            >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {sys.status === "connected" ? (
                      <Badge
                        variant="outline"
                        className="text-emerald-600 bg-emerald-50 border-emerald-200 text-[10px] h-5 px-1.5"
                      >
                        Connected
                      </Badge>
                    ) : (
                      <Badge
                        variant="outline"
                        className="text-red-600 bg-red-50 border-red-200 text-[10px] h-5 px-1.5"
                      >
                        Action Required
                      </Badge>
                    )}
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={() => onTestConnection?.(sys.name)}
                >
                  Test Connection
                </Button>
                {sys.status === "reauth" ? (
                  <Button
                    size="sm"
                    className="bg-[#E43632] hover:bg-[#C12E2A] text-white text-xs"
                    onClick={() => onReconnect?.(sys.name)}
                  >
                    Reconnect
                  </Button>
                ) : (
...
const NotificationsSettings = () => (
  <div className="space-y-6">
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
      <div className="space-y-4">
        <h4 className="font-bold text-[#0A0A0A] border-b border-gray-100 pb-2">
          Workspace Channels
        </h4>
...
// --- 6. Security ---
const SecuritySettings = ({ onExportCSV }: { onExportCSV?: () => void } = {}) => (
  <div className="space-y-6">
...
const BrandingSettings = ({
  onPublish,
  isSaving,
}: { onPublish?: () => void; isSaving?: boolean } = {}) => (
  <div className="space-y-6">
    <div className="flex justify-between items-center mb-2">
      <div>
        <h3 className="text-xl font-bold text-[#0A0A0A]">Custom Branding</h3>
        <p className="text-sm text-gray-500">
          Customize the look of contributor forms and PDF exports.
        </p>
      </div>
      <Button
        className="bg-[#0A0A0A] hover:bg-gray-800 text-white"
