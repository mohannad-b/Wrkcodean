...
export function ContributorsTab({ onInvite }: ContributorsTabProps = {}) {
    ...
    <div className="flex h-full bg-white">
      ...
      <div className="w-[350px] border-r border-gray-200 flex flex-col">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          ...
          <Button
            size="sm"
            className="h-8 w-8 rounded-full p-0 bg-[#E43632] hover:bg-[#C12E2A]"
            onClick={() => onInvite?.()}
          >
            <Plus size={16} />
          </Button>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span
                      className={`text-sm font-bold truncate ${selectedId === user.id ? "text-[#0A0A0A]" : "text-gray-700"}`}
                    >
                      {user.name}
                    </span>
                    {user.tasks > 0 && (
                      <Badge className="h-4 px-1 text-[9px] bg-red-50 text-[#E43632] border-red-100">
                        {user.tasks} Tasks
                      </Badge>
                    )}
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 flex items-start justify-between">
              <div className="flex items-start gap-5">
                <Avatar className="w-20 h-20 border-4 border-white shadow-lg">
                  <AvatarFallback className="bg-gray-100 text-2xl text-gray-400">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedUser.contributions.map((c, i) => (
                    <Badge
                      key={i}
                      variant="secondary"
                      className="bg-gray-100 text-gray-600 hover:bg-gray-200 cursor-pointer px-3 py-1.5"
                    >
                      {c}
                    </Badge>
                  ))}
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-3 bg-white p-3 rounded-lg border border-amber-100 shadow-sm">
                    ...
                    <span className="text-sm text-gray-700 font-medium">
                      Review Salesforce Connection Credentials
                    </span>
                  </div>
                  <div className="flex items-center gap-3 bg-white p-3 rounded-lg border border-amber-100 shadow-sm">
                    <div className="w-5 h-5 rounded border-2 border-gray-300" />
                    <span className="text-sm text-gray-700 font-medium">
                      Approve Logic Change for &gt;$10k
                    </span>
                  </div>
                </div>
              </div>
