...
export function TaskDrawer({ task, onClose, onSave, saving }: TaskDrawerProps) {
  ...
  useEffect(() => {
    ...
    const metaConnections = Array.isArray((task.metadata as any)?.secureConnections)
      ? ((task.metadata as any).secureConnections as SecureConnection[])
      : [];
    const fallbackName = (task.metadata as any)?.systemType ?? "Epic EHR";
    const fallbackConnector = (task.metadata as any)?.connectedBy ?? "Sarah C.";
              ...
              <div className="rounded-2xl border border-gray-200 bg-white shadow-sm p-4 space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div className="space-y-0.5">
                    <p className="text-sm font-semibold text-[#0A0A0A]">Connect accounts securely to allow this task to be automated.</p>
                    <p className="text-sm text-gray-500">Provide credentials via SSO or username & password.</p>
                  </div>
                  <Button
                    type="button"
                    onClick={() => setSystemPickerOpen(true)}
                    variant="outline"
                    className="h-11 rounded-xl border border-dashed border-gray-300 bg-white text-sm font-semibold text-[#0A0A0A] shadow-sm hover:border-gray-400"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {secureConnections.map((conn) => (
                    <div
                      key={conn.name}
                      className="flex items-center justify-between rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 shadow-inner"
                    >
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-600">
                          <KeyRound size={18} />
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-[#0A0A0A]">{conn.name}</p>
                          <p className="text-xs text-gray-500">Connected by {conn.connectedBy}</p>
                        </div>
                      </div>
                  ...
                  <button
                    type="button"
                    onClick={() => setSystemPickerOpen(true)}
                    className="flex items-center justify-center gap-2 rounded-2xl border border-dashed border-gray-300 bg-white px-4 py-3 text-sm font-semibold text-gray-700 hover:border-gray-400 hover:bg-gray-50 transition"
                  >
