...
export function ActivityTab({ onNavigateToBlueprint }: ActivityTabProps = {}) {
  ...
  return (
    <div className="h-full overflow-y-auto bg-gray-50">
      <div className="p-8 max-w-4xl mx-auto pb-32 space-y-6">
        ...
        <div className="space-y-6 relative before:absolute before:left-5 before:top-2 before:h-[calc(100%-20px)] before:w-px before:bg-gray-100">
          {filteredActivities.map((item) => (
            <div key={item.id} className="relative pl-12 group">
              <div
                className={`absolute left-0 top-0 w-10 h-10 rounded-full flex items-center justify-center border-4 border-white shadow-sm z-10 ${item.color}`}
              >
                <item.icon size={16} />
              </div>
              ...
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="secondary"
                      className="bg-gray-50 text-gray-700 hover:bg-gray-100 border-gray-200 font-mono text-[10px]"
                    >
                      {item.target}
                    </Badge>
                  </div>
